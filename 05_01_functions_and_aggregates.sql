...
-- NULL functions: IFNULL, NULLIF
  ...
  -- 01. COUNT – Total Employees
  -- Counts the total number of employees in the company
SELECT COUNT(*) AS total_employees
  FROM employees;


  -- 02. COUNT + GROUP BY – Employees per Department
...
GROUP BY dept_id;
  ...
  -- 08. HAVING + COUNT – Departments with 2+ Employees
...
FROM employees;
  ...
  -- 11.UPPER – Uppercase Conversion
  -- Converts first names to uppercase
SELECT 
    UPPER(first_name) AS uppercase_name
FROM employees;

  -- 12. LOWER – Lowercase Conversion
...
SELECT NOW() AS current_datetime;
  ...
  -- 15. DATE_FORMAT – Format Hire Date
...
FROM employees;
  ...
  -- 18. NULL HANDLING – IFNULL (DATA CLEANING QUERY)
...
FROM employees;
  ...
  -- 19. FILTERED GROUP ANALYSIS – HIGH SALARY DEPARTMENTS
...
-- 20. FILTERED DEPARTMENT ANALYSIS (GROUP BY + HAVING + ORDER BY)
